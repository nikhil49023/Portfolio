...
export const ProgressSlider: FC<ProgressSliderProps> = ({
  ...
  useEffect(() => {
    const getChildren = React.Children.toArray(children).find(
      (child) => React.isValidElement(child) && (child as React.ReactElement).type === SliderContent
    ) as React.ReactElement<{ children?: ReactNode }> | undefined;

    if (getChildren && getChildren.props && getChildren.props.children) {
      const values = React.Children.toArray(getChildren.props.children)
        .filter((child): child is React.ReactElement<{ value: string }> => 
          React.isValidElement(child) && Boolean((child.props as { value?: string })?.value)
        )
...
export const SliderBtn: FC<SliderBtnProps> = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        `relative transition-opacity duration-200 outline-none ${active === value ? 'opacity-100' : 'opacity-65 hover:opacity-90'}`,
        className
      )}
    ...
    >
      ...
      >
        <span
          className={cn('absolute left-0 transition-[width,height] ease-linear', progressBarClass)}
          style={{
